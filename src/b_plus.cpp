...
void b_plus::deleteTree(b_plus::node *start_node) {
  ...
  // If our current node is not a leaf node, visit all of its children first and
  // delete them, then delete this node
...
// This takes in a sorted vector of books and creates a linked list of leaf
// nodes Each leaf node contains at least ceil(order/2) values and at most
// order-1 values
void b_plus::createLeafs(vector<BooksFetcher::Book> &books) {
  ...
  for (int i = 1; i < books.size() / (order - 1); i++) {
    ...
    // If we are at the last full node, we check if there are still more books
    // at the end of the vector If so, we split the remaining books into two
    // smaller nodes
    if (i == books.size() / (order - 1) - 1 &&
        books.size() % (order - 1) != 0) {
      ...
      for (int k = 0; k < remaining_books - (remaining_books / 2); k++) {
        final_node->keys.push_back(
            books[k + (remaining_books / 2) + (i * (order - 1))]);
      }
    }
...
// This function creates the internal nodes of the b+ tree after the leaf nodes
...
void b_plus::createInternalNodes(vector<BooksFetcher::Book> &books) {
  ...
  // If there aren't enough leaves to create a full set of children for the
  // internal node, we store what leaves there are as children in a single
  // internal node
  if (num_nodes / order == 0) {
    node *new_internal = new node;
    for (int i = 0; i < num_nodes; i++) {
      // If the current leaf is not the final leaf, we add the book with the
      // smallest ISBN13 number in the next leaf to the keys of the internal
      // node
      if (i != num_nodes - 1) {
        new_internal->keys.push_back(current_node->next->keys[0]);
      }

      // We add the current leaf to the vector of children associated with the
      // internal node
  ...
  // Else, if there are enough leaves to create at least a single full set of
  // children for an internal node, we loop for the number of full sets that can
  // be made
  else {
    for (int i = 0; i < num_nodes / order; i++) {
      ...
      // If we are at the last full set of leaves, but there are additional
      // leaves that cannot form a full set, we split the remaining leaves in
      // half and create two new internal nodes This ensures that both internal
      // nodes have at least ceil(order/2) children
      if (i == (num_nodes / order) - 1 && num_nodes % order != 0) {
        ...
        for (int j = 0; j < remaining_nodes / 2; j++) {
          ...
          // We add the current leaf to the vector of children associated with
          // the internal node
          new_internal->children.push_back(current_node);
          current_node = current_node->next;
        }

        // We initialize the final node and add the remaining leaves to its
        // children and any relevant books to its keys
      ...
      // Else, if we can still make more full sets of leaves without any
      // remainders following it, we add a full internal node as usual
      else {
    ...
    // Once the first layer of internal nodes has been initialized, we call
    // fillInternalNodes to create more internal nodes leading to the root
    fillInternalNodes(first_layer);
  }
}

// This function takes in the first layer of internal nodes and builds up from
// it until a single root node is reached
void b_plus::fillInternalNodes(vector<b_plus::node *> &start_layer) {
  // current_layer is the current layer that is being filled with nodes that
  // point to the nodes in start_layer
  vector<node *> current_layer;

  // While there is still more than one node in the starting layer, we create an
  // additional layer of internal nodes to point to the starting layer
  while (start_layer.size() != 1) {

    // If there is not enough nodes in the starting layer to create a full set
    // of children, we place the remainder within a single internal node
    ...
    // Else, if there are enough nodes to create at least one full set of
    // children, we create as many full internal nodes as possible
    else {
      for (int i = 0; i < start_layer.size() / order; i++) {
        node *new_internal = new node;
        current_layer.push_back(new_internal);

        // If we are at the final set of full nodes but there are still
        // remaining nodes after it, we combine the remaining nodes and split
        // them among two internal nodes
        if (i == (start_layer.size() / order) - 1 &&
            start_layer.size() % order != 0) {
          int remaining_nodes = order + (start_layer.size() % order);

          for (int j = 0; j < remaining_nodes / 2; j++) {
            if (j != (remaining_nodes / 2) - 1) {
              new_internal->keys.push_back(
                  getSmallestBook(start_layer[(i * order) + j + 1]));
            }
          ...
          for (int k = 0; k < remaining_nodes - (remaining_nodes / 2); k++) {
            if (k != remaining_nodes - (remaining_nodes / 2) - 1) {
              final_internal->keys.push_back(getSmallestBook(
                  start_layer[(i * order) + k + (remaining_nodes / 2) + 1]));
            }

            final_internal->children.push_back(
                start_layer[(i * order) + k + (remaining_nodes / 2)]);
          }
        }

        // Else, if there are still more full sets of nodes without any
        // remainders after them, we create full internal nodes as usual
        else {
          for (int j = 0; j < order; j++) {
            if (j != order - 1) {
              new_internal->keys.push_back(
    ...
    // Once every node in the start_layer have become children to nodes in the
    // current_layer, we set the start_layer to the current_layer, clear the
    // current_layer, and repeat
...
// This function gets the book with the smallest ISBN13 number contained within
...
// Function to search through the tree for an indexed book, returning the node
// that book is located in
...
vector<vector<b_plus::node *>> b_plus::levelOrderTraverse() {
  ...
  // Calls recursiveLevelOrder to recursively traverse through the tree,
...
void b_plus::recursiveLevelOrder(b_plus::node *root_node,
                                 vector<vector<node *>> &traversal,
                                 int level_num) {
  // If this is the first node in the current level, initialize a new vector of
  // nodes to the traversal vector
  ...
  // If the current node is not a leaf node, add it to the current layer and
  // recursively traverse the child nodes in order
  traversal[level_num].push_back(root_node);
  for (int i = 0; i < root_node->children.size(); i++) {
    recursiveLevelOrder(root_node->children[i], traversal, level_num + 1);
  }

  // To separate groups of child nodes, a nullptr is inserted into the vector of
  // nodes once all the child nodes have been added to the vector
  traversal[level_num + 1].push_back(nullptr);
}

std::vector<Book> b_plus::collect() {
  std::vector<Book> res;
  b_plus::node *node = root;
  while (!node->isLeaf) {
    node = node->children[0];
  }

  while (node != nullptr) {
        res.insert(res.end(), node->keys.begin(), node->keys.end());
